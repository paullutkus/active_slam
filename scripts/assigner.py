...
def node():
        ...
        end_x = rospy.get_param('~end_x',3.0)
        end_y = rospy.get_param('~end_y',2.0)
...
#-------------------------------------------------------------------------  
                x = np.array([end_x, end_y])
                if (gridValue(mapData, x) != -1):
                        rospy.loginfo("ENDPOINT FOUND ENDPOINT FOUND ENDPOINT FOUND ENDPOINT FOUND")
                        rospy.signal_shutdown("found endpoint")
